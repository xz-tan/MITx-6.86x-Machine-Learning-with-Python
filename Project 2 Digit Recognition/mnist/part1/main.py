...
# print("softmax test_error when trained with mod 3 label: ", run_softmax_on_MNIST_mod3(temp_parameter=1))
#######################################################################
...
# def run_softmax_on_PCA_components(temp_parameter=1):
#     train_x, train_y, test_x, test_y = get_MNIST_data()
#     train_x_centered, feature_means = center_data(train_x)
#     pcs = principal_components(train_x_centered)
#     n_components = 18
...
#     return test_error
#
# print("Error rate for 18-dimensional PCA features = ", run_softmax_on_PCA_components(temp_parameter=1))
...
def run_softmax_on_PCA_components_and_cubic_features(temp_parameter=1):
    train_x, train_y, test_x, test_y = get_MNIST_data()
    train_x_centered, feature_means = center_data(train_x)
    pcs = principal_components(train_x_centered)
    n_components = 10
    cubic_train_pca = cubic_features(project_onto_PC(train_x, pcs, n_components, feature_means))
    cubic_test_pca = cubic_features(project_onto_PC(test_x, pcs, n_components, feature_means))
    theta, cost_function_history = softmax_regression(cubic_train_pca, train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4,
                                                      k=10, num_iterations=150)
    # plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(cubic_test_pca, test_y, theta, temp_parameter)
    return test_error
...
print("10-dimensional cubic PCA features", run_softmax_on_PCA_components_and_cubic_features(temp_parameter=1))