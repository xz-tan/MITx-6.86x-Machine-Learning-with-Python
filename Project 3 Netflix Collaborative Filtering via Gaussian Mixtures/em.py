    ...
    for n in range(X.shape[0]):
        ...
        if not(np.all(x == 0)):
            ...
            for K in range(len(mixture.mu)):
                ...
                prob_K = (p_K + 10 ** -16) * stats.multivariate_normal.pdf(x, mean=mu_K, cov=var_K)
                sum_prob_K += prob_K
                prob_K_list.append(math.log(prob_K + 10 ** -16))
...
def mstep(X: np.ndarray, post: np.ndarray, mixture: GaussianMixture,
    ...
    ### To calculate p_array
    p_array = np.sum(E_array_transpose, axis=1) / X.shape[0]  # Shape(K,)

    ### To calculate mu_array
    criteria_array =  E_array_transpose @ X_array  # Shape(1,d)
    not_pass_criteria_index = np.where(criteria_array < 1)
    mu_array = (E_array_transpose @ np.multiply(X_array, X)) / (E_array_transpose @ X_array)
    mu_array[not_pass_criteria_index] = mixture.mu[not_pass_criteria_index]

    ### To calculate var_array
    var_array = []
...
def fill_matrix(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    for n in range(X.shape[0]):
        ...
        if not (np.all(x == 0)):
            ...
            x = x[index_val_not_0]  # To get non-zero values
            for K in range(len(mixture.mu)):
                mu_K, var_K, p_K = mixture.mu[K][index_val_not_0], mixture.var[K], mixture.p[
                    K]  # Extract values of Mu that are non-zero
                prob_K = (p_K + 10 ** -16) * stats.multivariate_normal.pdf(x, mean=mu_K, cov=var_K)
                sum_prob_K += prob_K
                prob_K_list.append(math.log(prob_K + 10 ** -16))
            prob_K_list = np.array(prob_K_list)
    ...
    pred_matrix = E_array @ mixture.mu
    completed_matrix = np.where(X == 0, pred_matrix, X)
